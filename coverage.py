...
class SNAC(KMNC):
    def init_variable(self, hyper=None):
        assert hyper is None or isinstance(hyper, float)
        self.name = 'SNAC'
        self.range_dict = {}
        coverage_upper_dict = {}
        for (layer_name, layer_size) in self.layer_size_dict.items():
...
class TKNP(Coverage):
    def __init__(self, model, layer_size_dict, hyper=None):
        ...
        self.layer_pattern = {}  # ✅ Initialize BEFORE calling parent constructor
        
        super().__init__(model, layer_size_dict, hyper=hyper)  # ✅ Fix: Call AFTER initializing class attributes
    ...
    def init_variable(self, hyper):
        ...
        for layer_name in self.layer_size_dict.keys():
            self.layer_pattern[layer_name] = set()  # ✅ Fix: Ensure layer_pattern has all layers
    ...
    def calculate(self, data):
        ...
        for layer_name, layer_output in layer_output_dict.items():
            ...
            if layer_name not in self.layer_pattern:
                print(f"WARNING: {layer_name} not found in self.layer_pattern. Initializing it.")
                self.layer_pattern[layer_name] = set()  # ✅ Fix: Initialize missing layers properly
        ...
        network_pat = set.union(network_pat, self.network_pattern)  # ✅ Fix: Ensure self.network_pattern exists
...
class CC(Coverage): 
    """
    Cluster-based Coverage (CC), as proposed by TensorFuzz.
    """
    ...
    def init_variable(self, hyper):
        ...
        self.distant_dict = {}
        self.flann_dict = {}
        self.current = 0  # ✅ Fix: Initialize self.current to avoid AttributeError
...
# Ensure the script does not execute on import
